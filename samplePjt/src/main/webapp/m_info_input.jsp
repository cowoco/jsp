		...
		<script>
			$(function(){
				...
				$("#insertBtn").click(function(){
					...
					//var idPtn= /^[a-zA-Z4-9]{2,16}$/; //영문소문자 4~5자리 사이 값을 비교 패턴
					var idPtn= /^[a-z]{1}[a-zA-Z0-9]{2,15}$/; //첫글자는 영문소문, 다음부터는 영문자,숫자가능4~16
					//var namePtn= /^[가-힣]{1,3}$/; //국문 1~3자리까디 가능
					//var pwPtn= /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&*]).{4,20}$/; //영문자,숫자,특수문자1개 이상
					//var f_tellPtn= /^[0-9]{2,3}$/; //유선전화 가능 02,043,010
			...
			<form name="agreeFrm" method="post" action="doM_info_input.do">
				...
				<fieldset class="fieldset_class">
					...
					<dl id="join_tell_dl">
						...
						<dd>
							<input type="text" id="f_tell" name="f_tell" maxlength="3" value="${phone[0]}" required />
							<span> - </span>
							<input type="text" id="m_tell" name="m_tell" maxlength="4" value="${phone[1]}" required />
							<span> - </span>
							<input type="text" id="l_tell" name="l_tell" maxlength="4" value="${phone[2]}" required />
						</dd>
					</dl>
				...
				<fieldset class="fieldset_class">
					<dl id="join_interests_dl">
						...
						<dd>
							<ul>
								<li>
									<input type="checkbox" name="hobby" id="gmae" value="gmae" 
										<c:if test="${fn:contains(mdto.hobby,'gmae')}">checked</c:if>
									/>
									<label for="gmae">게임</label>
								</li>
								<li>
